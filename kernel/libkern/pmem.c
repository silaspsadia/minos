...
int mmap_first_free() {
	for (int i = 0; i < _pmem_total_blocks; i++) {
...
void allocate_chunk(uint32_t base_addr, int len) {
...
void pmem_init(struct multiboot_info *mb) {
	...
	allocate_chunk(KERNEL_START_PADDR, KERNEL_SIZE*KERNEL_SIZE);
	...
	printf("[Mem ] PMEM initialized to address range [%x, %x].\n", kernel_phys_map_start, kernel_phys_map_end);
}
